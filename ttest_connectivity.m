function [mcc_p,cpl_p,meloc_p,eglob_p,cc_ratio_p,cpl_ratio_p,sw_p,mod_p,strength_ps,cc_ps,bc_ps,eloc_ps,ereg_ps] = ttest_connectivity(strengths,cc,mcc,cpl,bc,ebc,eloc,meloc,ereg,eglob,small_worldness,cc_ratio,cpl_ratio,modularity,participation_thresh,Q_unthresh,strengths_pos,strengths_neg,participation_pos,participation_neg,diversity_pos,diversity_neg,num_regions,num_subjs,num_group1)
...
global_alpha=.05; % alpha for global measures
node_alpha=.01; % alpha for node measures
edge_alpha=.005; % alpha for edge measures
direction='both'; % right for group x > y, left for y > x
plot_metrics=1;
fdr_correct=false; % 1 to do False Discovery Rate correction (not well tested)
...
for i=1:num_regions
    ...
    [diversity_pos_ttest(i,1),diversity_pos_ttest(i,2)]=ttest2(diversity_pos(i,1:num_group1),diversity_pos(i,start_group2:num_subjs),node_alpha,direction);
    [diversity_neg_ttest(i,1),diversity_neg_ttest(i,2)]=ttest2(diversity_neg(i,1:num_group1),diversity_neg(i,start_group2:num_subjs),node_alpha,direction);
end
...
if fdr_correct
    ...
    diversity_pos_ttest(:,1)=diversity_pos_ttest(:,2)<=fdr(diversity_pos_ttest(:,2),fdr_q_level);
    diversity_neg_ttest(:,1)=diversity_neg_ttest(:,2)<=fdr(diversity_neg_ttest(:,2),fdr_q_level);
end

% Mean clustering coefficient
[mcc_ttest,mcc_p]=ttest2(mcc(1:num_group1),mcc(start_group2:num_subjs),global_alpha,direction)
mcc_grp1_grp2=[mean(mcc(1:num_group1)) mean(mcc(start_group2:num_subjs))];
...
if plot_metrics == 1
    ...
    title('Node unthreshold -participation')    
    subplot(3,2,5)
    imagesc(diversity_pos_ttest)
    title('Node unthreshold +diversity')    
    subplot(3,2,6)
    imagesc(diversity_neg_ttest)
    title('Node unthreshold -diversity')    
end